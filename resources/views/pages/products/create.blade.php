...
@section('title', 'Create Product | Zareena Industries')
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('unit_type') is-invalid @enderror" 
                                       id="unit_type" name="unit_type" value="{{ old('unit_type') }}" placeholder="e.g., kg, pcs, box">
                                @error('unit_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" class="form-control @error('opening_qty') is-invalid @enderror" 
                                       id="opening_qty" name="opening_qty" value="{{ old('opening_qty', 0) }}" min="0">
                                @error('opening_qty')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" class="form-control @error('current_qty') is-invalid @enderror" 
                                       id="current_qty" name="current_qty" value="{{ old('current_qty', 0) }}" min="0">
                                @error('current_qty')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
...
<script>
    ...
    // Auto-fill current_qty with opening_qty if current_qty is empty
    document.getElementById('opening_qty').addEventListener('input', function() {
        const currentQty = document.getElementById('current_qty');
        if (!currentQty.value || currentQty.value == 0) {
            currentQty.value = this.value;
        }
    });
</script>
@endpush
