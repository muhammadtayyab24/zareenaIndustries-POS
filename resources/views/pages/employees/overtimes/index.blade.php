...
@section('title', 'Overtime Hours | Zareena Industries')
        ...
        <div class="card">
            <div class="card-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h4 class="card-title">Overtime Hours</h4>
                    </div>
                </div>
            </div>
            <div class="card-body pt-0">
                ...
                <div class="card bg-light mb-4">
                    <div class="card-header">
                        <h5 class="card-title mb-0">Mark Overtime</h5>
                    </div>
                    <div class="card-body">
                        <div class="row align-items-end">
                            ...
                            <div class="col-md-3">
                                <label for="mark_ot_hours" class="form-label">OT Hours</label>
                                <input type="number" step="0.01" min="0" class="form-control" id="mark_ot_hours" placeholder="Enter hours">
                            </div>
                            <div class="col-md-3">
                                <button type="button" class="btn btn-primary w-100 mark-overtime-btn">
                ...
                <h5 class="mb-3">Overtime List</h5>
                        ...
                        <tbody>
                            @forelse($overtimes as $overtime)
                                <tr>
                                    ...
                                    <td>{{ \Carbon\Carbon::parse($overtime->date)->format('Y-m-d') }}</td>
                                    <td>{{ number_format($overtime->ot_hours, 2) }} hrs</td>
                                    <td class="text-end">
                                        <div class="d-flex gap-2 justify-content-end">
                                            <button class="btn btn-sm btn-primary edit-overtime" 
                                                    data-id="{{ $overtime->id }}"
                                                    data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button class="btn btn-sm btn-danger delete-overtime" 
                                                    data-id="{{ $overtime->id }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center">No overtime records found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            ...
            <div class="text-center py-3">
                <p class="mb-0 text-muted">Are you sure you want to delete this overtime record?</p>
            </div>
...
<script>
    ...
    // Mark Overtime functionality
    document.querySelector('.mark-overtime-btn').addEventListener('click', function() {
        const employeeId = document.getElementById('mark_employee_id').value;
        const date = document.getElementById('mark_date').value;
        const otHours = document.getElementById('mark_ot_hours').value;
        ...
        fetch('{{ route("employees.overtimes.store") }}', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                // Clear form
                document.getElementById('mark_ot_hours').value = '';
                // Reload page after a short delay to show updated data
                setTimeout(() => {
                    window.location.reload();
                }, 500);
            } else {
                alert(data.message || 'Error saving overtime');
                btn.disabled = false;
        ...
        .catch(error => {
            ...
            alert('An error occurred while saving overtime');
            btn.disabled = false;
            btn.innerHTML = `<span class="btn-text">${originalText}</span>`;
        });
    });
    ...
    // Delete Overtime
    document.querySelectorAll('.delete-overtime').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            deleteOvertimeId = this.getAttribute('data-id');
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
        ...
        fetch(`/employees/overtimes/${deleteOvertimeId}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json'
        ...
        .then(data => {
            if (data.success) {
                ...
                alert(data.message || 'Error deleting overtime');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            alert('An error occurred while deleting overtime');
        });
    });

    // Edit Overtime
    document.querySelectorAll('.edit-overtime').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            window.location.href = `/employees/overtimes/${this.getAttribute('data-id')}/edit`;
        });
    });
