...
@section('title', 'Expense Vouchers Management | Zareena Industries')
        ...
        <div class="card">
            <div class="card-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h4 class="card-title">Expense Vouchers Management</h4>
                    </div>
                    <div class="col-auto">
                        <a href="{{ route('expense-vouchers.create') }}" class="btn btn-primary">
                            <i class="fas fa-plus-circle me-1"></i> Create New Voucher
                        </a>
                    </div>
                </div>
            </div>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <div class="d-flex gap-2 justify-content-end">
                                            <a href="{{ route('expense-vouchers.edit', $voucher->id) }}"
                                                class="btn btn-sm btn-primary" data-bs-toggle="tooltip"
                                                data-bs-placement="top" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="#" class="btn btn-sm btn-danger delete-voucher"
                                                data-voucher-id="{{ $voucher->id }}"
                                                data-voucher-month="{{ $voucher->month }}" 
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            ...
            <div class="text-center py-3">
                <p class="mb-0 text-muted">Are you sure you want to delete expense voucher for <strong id="deleteVoucherInfo"></strong>?</p>                                                   
            </div><!--end modal-body-->
...
<script>
    // Delete Voucher
    document.querySelectorAll('.delete-voucher').forEach(function(deleteBtn) {
        deleteBtn.addEventListener('click', function(e) {
            e.preventDefault();
            const voucherId = this.getAttribute('data-voucher-id');
            const voucherMonth = this.getAttribute('data-voucher-month');
            const voucherAmount = this.getAttribute('data-voucher-amount');
            
            document.getElementById('deleteVoucherInfo').textContent = `${voucherMonth} (Amount: ${parseFloat(voucherAmount).toFixed(2)})`;
            document.getElementById('deleteForm').action = `/expense-vouchers/${voucherId}`;
